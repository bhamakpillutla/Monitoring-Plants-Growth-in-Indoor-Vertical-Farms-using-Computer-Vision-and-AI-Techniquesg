...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 23 17:23:56 2022

@author: Bhama Pillutla


Code to detect only leaves that are present in all timestamps of the day - centroid tracking 
Implements two stages of mask detections- one for first detections and second stage for tracking the same leaves.


Predictor file with zero centered centroid tracking :
    - tracks leaves in all timestamps of a day for 6 days 
    - includes a step to shift centroids to (0,0)
    
Outputs only tracked leaves
"""
...
while ix < len(all_images):
   ...
   for im_path in images:
       ...
       cv2.imwrite('/home/psych256lab/Downloads/Scripts/test_results/dummysample/'+str(d)+".jpg", out.get_image()[:,:,::-1])     
   ix = ix+6
   day = day + 1
dt = {'Date':csvDATE,'Day':csvDAY,'Time':csvTIME,'LeafIndex': csv_IDS,'Area':csvAREA,'Centroid':csvCENTROID}
df = pd.DataFrame(dt)
df.to_csv('/home/psych256lab/Downloads/Scripts/area_output(dummysample).csv')



""" uncomment for coco evaluation"""
